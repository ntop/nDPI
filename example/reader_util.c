...
int parse_proto_name_list(struct ndpi_global_context *g_ctx, char *str, int inverted_logic) {
  char *n;
  char op;
  ...
  for(n = strtok(str,_proto_delim); n && *n; n = strtok(NULL,_proto_delim)) {
    ...
    if(ndpi_set_protocol_enable(g_ctx, n, op) != NDPI_CFG_OK)
      LOG(NDPI_LOG_ERROR, "Error\n");
  }
