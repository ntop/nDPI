...
**IF** your own application wants to use `ndpi_malloc()` and similar functions also for its own allocations, `ndpiReader_install_memory_hooks()` **must** be called before **ANY** allocations, from the library and from the application, both. All these memory must be freed via `ndpi_free()`.
