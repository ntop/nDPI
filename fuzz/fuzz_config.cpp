...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  ...
  if(fuzzed_data.ConsumeBool()) {
    for(i = 0; i < 512; i++) { /* The number of internal protocols is not known at compile time. Use a reasonable guess */
      if(fuzzed_data.ConsumeBool())
        ndpi_set_protocol_enable_by_id(g_ctx, i, 0 /* disabled */);
    }
  }
  ...
  if(fuzzed_data.ConsumeBool()) {
    pid = fuzzed_data.ConsumeIntegralInRange<u_int16_t>(0, 512);
    value = fuzzed_data.ConsumeIntegralInRange(0, 1 + 1);
    snprintf(cfg_value, sizeof(cfg_value), "%d", value);
