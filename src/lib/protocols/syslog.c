			...
			*ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG__DEBUG(ndpi_struct, "search syslog\n");

  if (packet->payload_packet_len > 20 && packet->payload_packet_len <= 1024 && packet->payload[0] == '<') {
    NDPI_LOG__DEBUG2(ndpi_struct, "checked len>20 and <1024 and first symbol=<\n");
    ...
    if (packet->payload[i++] != '>') {
      NDPI_LOG__DEBUG(ndpi_struct, "excluded, there is no > following the number\n");
      NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_PROTOCOL_SYSLOG);
      return;
    } else {
      NDPI_LOG__DEBUG2(ndpi_struct, "a > following the number\n");
    }

    if (packet->payload[i] == 0x20) {
      NDPI_LOG__DEBUG2(ndpi_struct, "a blank following the >: increment i\n");
      i++;
    } else {
      NDPI_LOG__DEBUG2(ndpi_struct, "no blank following the >: do nothing\n");
    }
	...
	memcmp(packet->payload + i, "last message", sizeof("last message") - 1) == 0) {
      ...
      NDPI_LOG__TRACE(ndpi_struct, "found syslog by 'last message' string\n");
      ...
      NDPI_LOG__TRACE(ndpi_struct, "found syslog by 'snort: ' string\n");
	...
	&& memcmp(&packet->payload[i], "Nov", 3) != 0 && memcmp(&packet->payload[i], "Dec", 3) != 0) {
      ...
      NDPI_LOG__TRACE(ndpi_struct, "found syslog\n");
