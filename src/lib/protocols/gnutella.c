					     ...
					     /* ndpi_protocol_type_t protocol_type */)
{
  ...
  NDPI_LOG__TRACE(ndpi_struct, "found GNUTELLA\n");
...
void ndpi_search_gnutella(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG__DEBUG(ndpi_struct, "search GNUTELLA\n");
  ...
  if (packet->detected_protocol_stack[0] == NDPI_PROTOCOL_GNUTELLA) {
    if (src != NULL && ((u_int32_t)
      ...
      NDPI_LOG__DEBUG2(ndpi_struct, "save src connection packet detected\n");
      src->gnutella_ts = packet->tick_timestamp;
    } else if (dst != NULL && ((u_int32_t)
			       (packet->tick_timestamp - dst->gnutella_ts) < ndpi_struct->gnutella_timeout)) {
      NDPI_LOG__DEBUG2(ndpi_struct, "save dst connection packet detected\n");
      dst->gnutella_ts = packet->tick_timestamp;
    }
  ...
  if (packet->tcp != NULL) {
	...
	NDPI_LOG__DEBUG2(ndpi_struct, "detected GET /get/ or GET /uri-res/\n");
	ndpi_int_gnutella_add_connection(ndpi_struct, flow);
      }
    ...
    if (packet->payload_packet_len > 30 && memcmp(packet->payload, "HEAD /gnutella/push-proxy?", 26) == 0) {
      NDPI_LOG__DEBUG2(ndpi_struct, "detected HEAD /gnutella/push-proxy?\n");
      ndpi_int_gnutella_add_connection(ndpi_struct, flow);
      return;
    }
	...
	while (c < end) {
	  if ((memcmp(&packet->payload[14], "tigertree/", 10) == 0)
	    ...
	    NDPI_LOG__DEBUG2(ndpi_struct, "FOXY :: GNUTELLA GET 2 DETECTED\n");
	    ndpi_int_gnutella_add_connection(ndpi_struct, flow);
	    return;
	  }
	...
	NDPI_LOG__DEBUG2(ndpi_struct, "MORPHEUS GIV DETECTED\n");
	...
	get_u_int16_t(packet->payload, 12) == htons(0x0034)) {
      NDPI_LOG__DEBUG2(ndpi_struct, "detected gnutella len == 46\n");
      ndpi_int_gnutella_add_connection(ndpi_struct, flow);
      return;
    }
	...
	memcmp(packet->payload, "\x80\x2f\x01\x03\x01\x00\x06\x00\x00\x00\x20\x00\x00\x34\x00\x00\xff\x4d\x6c",
	       19) == 0) {
      NDPI_LOG__DEBUG2(ndpi_struct, "detected gnutella len == 49\n");
      ndpi_int_gnutella_add_connection(ndpi_struct, flow);
      return;
    }
	...
	NDPI_LOG__DEBUG2(ndpi_struct, "detected len == 82\n");
	ndpi_int_gnutella_add_connection(ndpi_struct, flow);
	return;
      }
    }
	...
	(packet->tick_timestamp - src->gnutella_ts) < ndpi_struct->gnutella_timeout) {
      NDPI_LOG__DEBUG2(ndpi_struct, "port based detection\n\n");
      ndpi_int_gnutella_add_connection(ndpi_struct, flow);
    }
    /* observations:
	...
	&& packet->payload[20] == 0x00 && packet->payload[21] == 0x00 && packet->payload[22] == 0x00) {
      NDPI_LOG__DEBUG2(ndpi_struct, "detected gnutella udp, len = 23\n");
      ndpi_int_gnutella_add_connection(ndpi_struct, flow);
	...
	NDPI_LOG__DEBUG2(ndpi_struct, "detected gnutella udp, GND\n");
	ndpi_int_gnutella_add_connection(ndpi_struct, flow);
	return;
      }
    }
	...
	&& memcmp(&packet->payload[16], "\x31\x01\x00\x09\x00\x00\x00", 7) == 0) {
      NDPI_LOG__DEBUG2(ndpi_struct, "detected gnutella udp, len = 32 ii\n");
      ndpi_int_gnutella_add_connection(ndpi_struct, flow);
      return;
    }
    if ((packet->payload_packet_len == 23)
	&& memcmp(&packet->payload[16], "\x00\x01\x00\x00\x00\x00\x00", 7) == 0) {
      NDPI_LOG__DEBUG2(ndpi_struct, "detected gnutella udp, len = 23 ii\n");
      ndpi_int_gnutella_add_connection(ndpi_struct, flow);
      return;
    }
  }
  ...
  if (packet->tcp != NULL && ntohs(packet->tcp->source) >= 1024 && ntohs(packet->tcp->dest) >= 1024) {
	...
	NDPI_LOG__DEBUG2(ndpi_struct, "GNUTELLA DETECTED due to message ID match (NEONet protocol)\n");
	ndpi_int_gnutella_add_connection(ndpi_struct, flow);
	return;
      }
