...
static void ndpi_http_check_human_redeable_content(struct ndpi_flow_struct *flow,
...
static void ndpi_validate_http_content(struct ndpi_detection_module_struct *ndpi_struct,
  ...
  if(double_ret) {
	...
	ndpi_http_check_human_redeable_content(flow, double_ret, len);
      }
    }
...
static void check_content_type_and_change_protocol(struct ndpi_detection_module_struct *ndpi_struct,
  ...
  if(packet->user_agent_line.ptr != NULL) {
    http_process_user_agent(flow, packet->user_agent_line.ptr, packet->user_agent_line.len);
  }
...
static void reset(struct ndpi_flow_struct *flow) {
