...
void ndpi_search_dhcp_udp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG__DEBUG(ndpi_struct, "search DHCP\n");
...
#ifdef DHCP_DEBUG
	  NDPI_LOG__DEBUG2(ndpi_struct, "[DHCP] Id=%d [len=%d]\n", id, len);
#endif
...
#ifdef DHCP_DEBUG
	    NDPI_LOG__DEBUG2(ndpi_struct, "[DHCP] '%.*s'\n",name,len);
//	    while(j < len) { printf( "%c", name[j]); j++; }; printf("\n");
	...
	NDPI_LOG__TRACE(ndpi_struct, "found DHCP\n");
