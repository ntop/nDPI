...
void ndpi_search_fiesta(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	NDPI_LOG__DEBUG(ndpi_struct, "search fiesta\n");
	...
	if (flow->l4.tcp.fiesta_stage == 0 && packet->payload_packet_len == 5
		...
		NDPI_LOG__DEBUG2(ndpi_struct, "maybe fiesta symmetric, first packet\n");
	...
	if (flow->l4.tcp.fiesta_stage == (2 - packet->packet_direction)
		...
		NDPI_LOG__DEBUG2(ndpi_struct, "Maybe fiesta\n");
		goto maybe_fiesta;
	}
  ...
  maybe_fiesta:
	NDPI_LOG__DEBUG2(ndpi_struct, "Stage is set to %d\n", flow->l4.tcp.fiesta_stage);
	return;

  add_fiesta:
	NDPI_LOG__TRACE(ndpi_struct, "found fiesta\n");
