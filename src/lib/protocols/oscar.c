...
//  u_int8_t channel;
  ...
  if (packet->payload_packet_len >= 6 && packet->payload[0] == 0x2a)
	  ...
	  get_u_int32_t(packet->payload, 6) == htonl(FLAPVERSION))
	{
	  ...
	  if(packet->payload_packet_len == 10)
	    {
	      NDPI_LOG__TRACE(ndpi_struct, "found OSCAR - Sign In \n");
	  ...
	  /*   { */
	  /*     NDPI_LOG__TRACE(ndpi_struct, "found OSCAR - Screen Name \n"); */
	  /*     ndpi_int_oscar_add_connection(ndpi_struct, flow); */
	  /*     return; */
	  /*   } */
	  /* /\* PASSWD *\/ */
	  ...
	  /*   { */
	  /*     NDPI_LOG__TRACE(ndpi_struct, "found OSCAR - Password (roasted) \n"); */
	  /*     ndpi_int_oscar_add_connection(ndpi_struct, flow); */
	  /*     return; */
	  /*   } */
	  /* CLIENT_NAME */
	  if (get_u_int16_t(packet->payload, 10) == htons(CLIENT_NAME)) /* packet->payload[10] == 0x00 && packet->payload[11] == 0x03 */
	    {
	      NDPI_LOG__TRACE(ndpi_struct, "found OSCAR - Client Name \n");
	      ndpi_int_oscar_add_connection(ndpi_struct, flow);
	      return;
	    }
	  /* LOGIN_COOKIE */
	      ...
	      if(get_u_int16_t(packet->payload, packet->payload_packet_len - 5) == htons(MULTICONN_FLAGS)) /* MULTICONN_FLAGS */
		{
		  if(get_u_int16_t(packet->payload, packet->payload_packet_len - 3) == htons(0x0001))
		    if((get_u_int8_t(packet->payload, packet->payload_packet_len - 1) == 0x00) ||
		      ...
		      {
			NDPI_LOG__TRACE(ndpi_struct, "found OSCAR - Login \n");
			ndpi_int_oscar_add_connection(ndpi_struct, flow);
			return;
		      }
		}
	  ...
	  if (get_u_int16_t(packet->payload, 10) == htons(MAJOR_VERSION))
	    {
	      NDPI_LOG__TRACE(ndpi_struct, "found OSCAR - Major_Version \n");
	      ndpi_int_oscar_add_connection(ndpi_struct, flow);
	      return;
	    }
	  /* MINOR_VERSION */
	  if (get_u_int16_t(packet->payload, 10) == htons(MINOR_VERSION))
	    {
	      NDPI_LOG__TRACE(ndpi_struct, "found OSCAR - Minor_Version \n");
	      ndpi_int_oscar_add_connection(ndpi_struct, flow);
	      return;
	    }
	  /* POINT_VERSION */
	  if (get_u_int16_t(packet->payload, 10) == htons(POINT_VERSION))
	    {
	      NDPI_LOG__TRACE(ndpi_struct, "found OSCAR - Point_Version \n");
	      ndpi_int_oscar_add_connection(ndpi_struct, flow);
	      return;
	    }
	  /* BUILD_NUM */
	  if (get_u_int16_t(packet->payload, 10) == htons(BUILD_NUM))
	    {
	      NDPI_LOG__TRACE(ndpi_struct, "found OSCAR - Build_Num \n");
	      ndpi_int_oscar_add_connection(ndpi_struct, flow);
	      return;
	    }
	  /* CLIENT_RECONNECT */
	  if (get_u_int16_t(packet->payload, 10) == htons(CLIENT_RECONNECT))
	    {
	      NDPI_LOG__TRACE(ndpi_struct, "found OSCAR - Client_Reconnect \n");
	      ndpi_int_oscar_add_connection(ndpi_struct, flow);
	      return;
	    }
	}
      ...
      if (channel == DATA)
	{
	  ...
	  if (packet->payload_packet_len >= 12)
	  {
	        ...
	        if (packet->payload_packet_len >= 16)
		{
		  ...
		  if((req_ID <= ((u_int32_t)-1)))
		    {
		      NDPI_LOG__TRACE(ndpi_struct, "found OSCAR\n");
		      ndpi_int_oscar_add_connection(ndpi_struct, flow);
		      return;
		    }
		}
      ...
      */
      if (channel == O_ERROR)
	{
	  NDPI_LOG__TRACE(ndpi_struct, "found OSCAR - Error frame \n");
	  ndpi_int_oscar_add_connection(ndpi_struct, flow);
	  return;
	}
      /*
      ...
      */
      if (channel == SIGNOFF)
	{
	  NDPI_LOG__TRACE(ndpi_struct, "found OSCAR - Signoff frame \n");
	  ndpi_int_oscar_add_connection(ndpi_struct, flow);
	  return;
	}
      /*
	...
	{
	  NDPI_LOG__TRACE(ndpi_struct, "found OSCAR - Keep Alive frame \n");
	  ndpi_int_oscar_add_connection(ndpi_struct, flow);
	  return;
	}
    }
  ...
  if (packet->payload_packet_len > 40) {
	...
	NDPI_LOG__TRACE(ndpi_struct, "found OSCAR over HTTP, GET /aim/\n");
	ndpi_int_oscar_add_connection(ndpi_struct, flow);
	return;
      }
	...
	if (packet->user_agent_line.len > 15 && packet->user_agent_line.ptr != NULL &&
	  ...
	  NDPI_LOG__TRACE(ndpi_struct, "found OSCAR over HTTP\n");
	  ndpi_int_oscar_add_connection(ndpi_struct, flow);
	  return;
	}
      }
	...
	NDPI_LOG__TRACE(ndpi_struct, "found OSCAR ICQ-HTTP\n");
	ndpi_int_oscar_add_connection(ndpi_struct, flow);
	return;
      }
      if (memcmp(packet->payload, "CONNECT login.oscar.aol.com:5190 HTTP/1.", 40) == 0) {
	NDPI_LOG__TRACE(ndpi_struct, "found OSCAR AIM-HTTP\n");
	ndpi_int_oscar_add_connection(ndpi_struct, flow);
	return;
      }
  ...
  if (packet->payload_packet_len > 43
    ...
    NDPI_LOG__TRACE(ndpi_struct, "found OSCAR ICQ-HTTP PROXY\n");
    ndpi_int_oscar_add_connection(ndpi_struct, flow);
    return;
  }
  ...
  if (packet->payload_packet_len > 46
    ...
    NDPI_LOG__TRACE(ndpi_struct, "found OSCAR AIM-HTTP PROXY\n");
    ndpi_int_oscar_add_connection(ndpi_struct, flow);
    return;
  }

  if (packet->payload_packet_len > 5 && get_u_int32_t(packet->payload, 0) == htonl(0x05010003)) {
    NDPI_LOG__DEBUG2(ndpi_struct, "Maybe OSCAR Picturetransfer\n");
    return;
  }
  ...
  if (packet->payload_packet_len >= 70 &&
    ...
    NDPI_LOG__TRACE(ndpi_struct, "found OSCAR PICTURE TRANSFER\n");
    ndpi_int_oscar_add_connection(ndpi_struct, flow);
    return;
  }
  ...
  if (NDPI_SRC_OR_DST_HAS_PROTOCOL(src, dst, NDPI_PROTOCOL_OSCAR) != 0) {
	...
	NDPI_LOG__TRACE(ndpi_struct, "found OSCAR FILE TRANSFER\n");
	ndpi_int_oscar_add_connection(ndpi_struct, flow);
	return;
      }
...
{
  struct ndpi_packet_struct *packet = &flow->packet;
  NDPI_LOG__DEBUG(ndpi_struct, "search OSCAR\n");
  if (packet->tcp != NULL) {
