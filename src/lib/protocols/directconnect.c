				    ...
				    *ndpi_struct, const u_int8_t * payload, int payload_len)
{
  ...
  while (i < payload_len) {
	...
	if (memcmp(&payload[i + 15], "ADCS", 4) == 0) {
	  ...
	  NDPI_LOG__DEBUG2(ndpi_struct, "DC ssl port parsed %d\n", ssl_port);
	}
      }
    } else {
      break;
...
static void ndpi_search_directconnect_tcp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (src != NULL) {
	...
	src->directconnect_last_safe_access_time = packet->tick_timestamp;
	NDPI_LOG__TRACE(ndpi_struct, "found DC using port %d\n", ntohs(src->detected_directconnect_port));
	...
	NDPI_LOG__DEBUG2(ndpi_struct, "resetting src port due to timeout\n");
	return;
      }
    }
  ...
  if (dst != NULL) {
	...
	NDPI_LOG__DEBUG(ndpi_struct, "resetting dst port due to timeout\n");
	return;
      }
    }
    if (dst->detected_directconnect_ssl_port == packet->tcp->dest) {
	  ...
	  (packet->tick_timestamp -
	   dst->directconnect_last_safe_access_time) < ndpi_struct->directconnect_connection_ip_tick_timeout) {
	dst->directconnect_last_safe_access_time = packet->tick_timestamp;
	NDPI_LOG__DEBUG(ndpi_struct, "found DC using port %d\n", ntohs(dst->detected_directconnect_ssl_port));
	ndpi_set_detected_protocol(ndpi_struct, flow, NDPI_PROTOCOL_DIRECTCONNECT, NDPI_PROTOCOL_UNKNOWN);
	return;
      } else {
	dst->detected_directconnect_ssl_port = 0;
	NDPI_LOG__DEBUG2(ndpi_struct, "resetting dst port due to timeout\n");
	return;
      }
    }
  ...
  if (flow->directconnect_stage == 0) {
	...
	NDPI_LOG__DEBUG2(ndpi_struct, "maybe first dc connect to hub  detected\n");
	...
	NDPI_LOG__DEBUG2(ndpi_struct, "maybe first dc connect between peers  detected\n");
	...
	NDPI_LOG__TRACE(ndpi_struct, "found DC HSUP ADBAS0 E\n");
	...
	NDPI_LOG__TRACE(ndpi_struct, "found DC CSUP ADBAS0 E\n");
    ...
    if (packet->payload_packet_len >= 11) {
      ...
      if (memcmp(&packet->payload[0], "HSUP ADBAS0", 11) == 0
	  || memcmp(&packet->payload[0], "HSUP ADBASE", 11) == 0) {
	NDPI_LOG__TRACE(ndpi_struct, "found DC HSUP ADBAS E in second packet\n");
	ndpi_int_directconnect_add_connection(ndpi_struct, flow, DIRECT_CONNECT_TYPE_HUB);
	return;
	/* did not see this pattern in any trace */
      } else if (memcmp(&packet->payload[0], "CSUP ADBAS0", 11) == 0 ||
		 memcmp(&packet->payload[0], "CSUP ADBASE", 11) == 0) {
	NDPI_LOG__TRACE(ndpi_struct, "found DC HSUP ADBAS0 E in second packet\n");
	ndpi_int_directconnect_add_connection(ndpi_struct, flow, DIRECT_CONNECT_ADC_PEER);
	return;
    ...
    if (packet->payload_packet_len > 6) {
	  ...
	  && packet->payload[packet->payload_packet_len - 1] == '|') {
	NDPI_LOG__TRACE(ndpi_struct, "found DC second\n");
	ndpi_int_directconnect_add_connection(ndpi_struct, flow, DIRECT_CONNECT_TYPE_HUB);
	return;
      } else {
	NDPI_LOG__DEBUG2(ndpi_struct, "second dc not detected\n");
      }
	...
	NDPI_LOG__TRACE(ndpi_struct, "found DC between peers\n");
	ndpi_int_directconnect_add_connection(ndpi_struct, flow, DIRECT_CONNECT_TYPE_PEER);
	return;
      } else {
	NDPI_LOG__DEBUG2(ndpi_struct, "second dc between peers not detected\n");
      }
    }
					  ...
					  *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (dst != NULL && dst->detected_directconnect_udp_port == packet->udp->dest) {
	...
	(packet->tick_timestamp -
	 dst->directconnect_last_safe_access_time) < ndpi_struct->directconnect_connection_ip_tick_timeout) {
      ...
      dst->directconnect_last_safe_access_time = packet->tick_timestamp;
      NDPI_LOG__TRACE(ndpi_struct, "found DC using udp port %d\n", ntohs(dst->detected_directconnect_udp_port));
      ndpi_set_detected_protocol(ndpi_struct, flow, NDPI_PROTOCOL_DIRECTCONNECT, NDPI_PROTOCOL_UNKNOWN);
      return;
    } else {
      dst->detected_directconnect_udp_port = 0;
      NDPI_LOG__DEBUG2(ndpi_struct, "resetting dst udp  port due to timeout\n");
      return;
    }
  }
	...
	if (packet->payload[pos] == ')') {
	  ...
	  if (packet->payload[pos] == '(') {
	    ...
	    if (pos > 2 && memcmp(&packet->payload[pos], "TTH:", 4) == 0) {
	      NDPI_LOG__TRACE(ndpi_struct, "found DC udp\n");
	      ndpi_int_directconnect_add_connection(ndpi_struct, flow, DIRECT_CONNECT_TYPE_PEER);
	      return;
	    }
	  }
  ...
  NDPI_LOG__DEBUG(ndpi_struct, "excluded DC at stage %d \n", flow->directconnect_stage);
			       ...
			       *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG__DEBUG(ndpi_struct, "search DC\n");
  ...
  if (packet->detected_protocol_stack[0] == NDPI_PROTOCOL_DIRECTCONNECT) {
    if (src != NULL && ((u_int32_t)
      ...
      NDPI_LOG__DEBUG2(ndpi_struct, "skipping as unknown due to timeout\n");
    }
    return;
  }
