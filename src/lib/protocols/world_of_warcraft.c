				 ...
				 *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG__DEBUG(ndpi_struct, "search World of Warcraft\n");
  ...
  if (packet->tcp != NULL) {
    /*
      ...
      NDPI_LOG__DEBUG(ndpi_struct, "World of Warcraft: Web Client found\n");
      return;
      }
      }
    */
    ...
    if (packet->payload_packet_len == 50 && memcmp(&packet->payload[2], "WORLD OF WARCRAFT CONNECTION",
      ...
      ndpi_int_worldofwarcraft_add_connection(ndpi_struct, flow);
      NDPI_LOG__TRACE(ndpi_struct, "World of Warcraft: Login found\n");
      return;
    }
	...
	ndpi_int_worldofwarcraft_add_connection(ndpi_struct, flow);
	NDPI_LOG__TRACE(ndpi_struct, "World of Warcraft: connection detected\n");
	return;
      }
	...
	if (get_u_int32_t(packet->payload, 2) == htonl(0xec010100)) {
	  ...
	  NDPI_LOG__DEBUG2(ndpi_struct, "probably World of Warcraft, waiting for final packet\n");
	  ...
	  NDPI_LOG__DEBUG2(ndpi_struct, "maybe World of Warcraft, need next\n");
	...
	NDPI_LOG__DEBUG2(ndpi_struct, "waiting for final packet\n");
	return;
      }
    }
