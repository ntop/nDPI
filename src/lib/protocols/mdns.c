...
static int ndpi_int_check_mdns_payload(struct ndpi_detection_module_struct
  ...
  if(((packet->payload[2] & 0x80) == 0)
    ...
    NDPI_LOG__TRACE(ndpi_struct, "found MDNS with question query\n");
    return 1;    
  }
    ...
    NDPI_LOG__TRACE(ndpi_struct, "found MDNS with answer query\n");
    return 1;
  }
...
void ndpi_search_mdns(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG__DEBUG(ndpi_struct, "search MDNS\n");
  ...
  */
  ...
  if(packet->udp != NULL) {   
	...
	NDPI_LOG__TRACE(ndpi_struct, "found MDNS with destination address 224.0.0.251 (=0xe00000fb)\n");
	...
	if(daddr[0] == htonl(0xff020000) /* && daddr[1] == 0 && daddr[2] == 0 && daddr[3] == htonl(0xfb) */) {
	  ...
	  NDPI_LOG__TRACE(ndpi_struct, "found MDNS with destination address ff02::fb\n");
