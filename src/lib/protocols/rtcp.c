...
void ndpi_search_rtcp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG__DEBUG(ndpi_struct, "search RTCP\n");

  if(packet->tcp != NULL) {
    sport = ntohs(packet->tcp->source), dport = ntohs(packet->tcp->dest);
    NDPI_LOG__DEBUG2(ndpi_struct, "calculating dport over tcp\n");
    ...
    if(packet->payload_packet_len > 13 && (sport == 554 || dport == 554) &&
      ...
      NDPI_LOG__TRACE(ndpi_struct, "found rtcp\n");
    ...
    NDPI_LOG__DEBUG2(ndpi_struct, "calculating dport over udp\n");
