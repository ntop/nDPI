...
#define PROTO_EDONKEY        0xE3
#define PROTO_EMULE_EXT      0xC5
#define PROTO_EMULE_COMP     0xD4

static void ndpi_int_edonkey_add_connection(struct ndpi_detection_module_struct *ndpi_struct, 
...
static void ndpi_search_edonkey(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (packet->payload_packet_len > 5) {
    u_int8_t protocol = packet->payload[0];
    if (protocol != PROTO_EDONKEY && protocol != PROTO_EMULE_EXT &&
        protocol != PROTO_EMULE_COMP)
    {
      NDPI_EXCLUDE_PROTO(ndpi_struct, flow);
      return;
    }
    /* All eDonkey over TCP packets should have this field */
    u_int32_t message_length = packet->payload_packet_len-5;
