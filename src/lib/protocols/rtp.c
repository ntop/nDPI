...
{
  NDPI_LOG__DEBUG(ndpi_struct, "search RTP\n");
  if (payload_len < 2)
    return;
  ...
  if((payload_len >= 12)
    ...
    NDPI_LOG__TRACE(ndpi_struct, "Found RTP\n");
    ...
    if(payloadType == 1 /* RTP */) {
      NDPI_LOG__TRACE(ndpi_struct, "Found Skype for Business (former MS Lync)\n");
      ndpi_set_detected_protocol(ndpi_struct, flow, NDPI_PROTOCOL_SKYPE, NDPI_PROTOCOL_UNKNOWN);
      return;
    } else /* RTCP */ {
      NDPI_LOG__TRACE(ndpi_struct, "Found MS RTCP\n");
	      ...
	      u_int8_t direction, u_int16_t seq, u_int8_t include_current_packet)
{
  ...
  NDPI_LOG__DEBUG(ndpi_struct, "rtp_seqnum[%u] = %u\n", direction, seq);
}
		     ...
		     u_int8_t direction, u_int16_t seq)
{
  ...
  if (delta < RTP_MAX_OUT_OF_ORDER) {	/* in order, with permissible gap */
    flow->rtp_seqnum[direction] = seq;
    NDPI_LOG__DEBUG(ndpi_struct, "rtp_seqnum[%u] = %u (increased by %u)\n",
	     direction, seq, delta);
    return delta;
  } else {
    NDPI_LOG__DEBUG(ndpi_struct, "retransmission (dir %u, seqnum %u)\n",
	     direction, seq);
    return 0;
  }
}
			    ...
			    const u_int8_t * payload, const u_int16_t payload_len)
{
  ...
  if (payload_len == 4 && get_u_int32_t(packet->payload, 0) == 0 && flow->packet_counter < 8) {
    NDPI_LOG__DEBUG(ndpi_struct, "need next packet, maybe ClearSea out calls\n");
    return;
  }
  ...
  if (payload_len == 3 && memcmp(payload, "png", 3) == 0) {
    ...
    NDPI_LOG__DEBUG(ndpi_struct, "skipping packet with len = 3 and png payload\n");
    return;
  }

  if (payload_len < 12) {
    NDPI_LOG__DEBUG(ndpi_struct, "minimal packet size for rtp packets: 12\n");
    goto exclude_rtp;
  }

  if (payload_len == 12 && get_u_int32_t(payload, 0) == 0 && get_u_int32_t(payload, 4) == 0 && get_u_int32_t(payload, 8) == 0) {
    NDPI_LOG__DEBUG(ndpi_struct, "skipping packet with len = 12 and only 0-bytes\n");
    return;
  }

  if ((payload[0] & 0xc0) == 0xc0 || (payload[0] & 0xc0) == 0x40 || (payload[0] & 0xc0) == 0x00) {
    NDPI_LOG__DEBUG(ndpi_struct, "version = 3 || 1 || 0, maybe first rtp packet\n");
    return;
  }

  if ((payload[0] & 0xc0) != 0x80) {
    NDPI_LOG__DEBUG(ndpi_struct, "rtp version must be 2, first two bits of a packets must be 10\n");
    goto exclude_rtp;
  }
  ...
  if (flow->rtp_payload_type[packet->packet_direction] != (payload[1] & 0x7F)) {
    NDPI_LOG__DEBUG(ndpi_struct, "payload_type has changed, reset stages\n");
  ...
  if (stage > 0) {
    ...
    NDPI_LOG__DEBUG(ndpi_struct, "rtp_ssid[%u] = %u\n", packet->packet_direction,
	     flow->rtp_ssid[packet->packet_direction]);
    flow->rtp_ssid[packet->packet_direction] = get_u_int32_t(payload, 8);
    if (flow->packet_counter < 3) {
      NDPI_LOG__DEBUG(ndpi_struct, "packet_counter < 3, need next packet\n");
    }
    init_seq(ndpi_struct, flow, packet->packet_direction, seqnum, 1);
  }
  if (seqnum <= 3) {
    NDPI_LOG__DEBUG(ndpi_struct, "sequence_number = %u, too small, need next packet, return\n", seqnum);
    return;
  }

  if (stage == 3) {
    NDPI_LOG__DEBUG(ndpi_struct, "add connection I\n");
    ndpi_int_rtp_add_connection(ndpi_struct, flow);
  } else {
    packet->packet_direction == 0 ? flow->rtp_stage1++ : flow->rtp_stage2++;
    NDPI_LOG__DEBUG(ndpi_struct, "stage[%u]++; need next packet\n",
	     packet->packet_direction);
  }
  return;
 ...
 exclude_rtp:
  ...
  if (packet->detected_protocol_stack[0] == NDPI_PROTOCOL_STUN
    ...
    NDPI_LOG__DEBUG(ndpi_struct, "STUN: is detected, need next packet\n");
    return;
  }
...
void ndpi_search_rtp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (packet->udp) {
    ...
    if (NDPI_FLOW_PROTOCOL_EXCLUDED(ndpi_struct, flow, NDPI_PROTOCOL_STUN)) {
      ...
      NDPI_LOG__DEBUG(ndpi_struct, "STUN not yet excluded, need next packet\n");
    }
#else
    NDPI_EXCLUDE_PROTO(ndpi_struct, flow);
#endif
