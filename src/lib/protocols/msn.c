...
static void ndpi_search_msn_tcp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if(packet->detected_protocol_stack[0] == NDPI_PROTOCOL_SSL) {
    ...
    NDPI_LOG__DEBUG2(ndpi_struct, "msn ssl ft test\n");
	...
	if(memcmp(packet->payload + 24, "MSNSLP", 6) == 0
	  ...
	  NDPI_LOG__TRACE(ndpi_struct, "found MSN File Transfer, ifdef ssl\n");
  ...
  if(flow->packet_counter == 1
	...
	if (ndpi_match_strprefix(packet->payload, packet->payload_packet_len, "VER ")) {
	  ...
	  if (memcmp(&packet->payload[packet->payload_packet_len - 6], "CVR",
	    ...
	    NDPI_LOG__TRACE(ndpi_struct, "found MSN by pattern VER...CVR/MSNP ODOA\n");
	    ndpi_int_msn_add_connection(ndpi_struct, flow);
	    return;
	  }
	  if (ndpi_match_strprefix(&packet->payload[4], packet->payload_packet_len-4, "MSNFT")) {
	    NDPI_LOG__TRACE(ndpi_struct, "found MSN FT by pattern VER MSNFT...0d0a\n");
	    ndpi_int_msn_add_connection(ndpi_struct, flow);
	    return;
	  }
	}
	...
	NDPI_LOG__TRACE(ndpi_struct, "found MSN Messenger/\n");
	ndpi_int_msn_add_connection(ndpi_struct, flow);
	return;
      }
    }
...
/* #endif */
	...
	if(packet->content_line.ptr != NULL &&
	  ...
	  NDPI_LOG__TRACE(ndpi_struct, "found MSN POST application/x-msn-messenger\n");
	  ndpi_int_msn_add_connection(ndpi_struct, flow);
	  return;
	}
      }
	...
	if(memcmp(&packet->payload[5], "http://", 7) == 0) {
	  ...
	  for(c = 13; c < 50; c++) {
		...
		NDPI_LOG__DEBUG2(ndpi_struct, "found  pattern http://.../gateway/gateway.ddl\n");
		status = 1;
		break;
	      }
	    }
	...
	if(packet->content_line.ptr != NULL && ((packet->content_line.len == 23
	  ...
	  if ((src != NULL && NDPI_COMPARE_PROTOCOL_TO_BITMASK(src->detected_protocol_bitmask, NDPI_PROTOCOL_MSN) != 0)
	    ...
	    NDPI_LOG__TRACE(ndpi_struct, "found MSN with pattern text/xml; charset=utf-8\n");
	    ndpi_int_msn_add_connection(ndpi_struct, flow);
	    return;
	  }
	  for(a = 0; a < packet->parsed_lines; a++) {
	    if(packet->line[a].len >= 4 && (memcmp(packet->line[a].ptr, "CVR ", 4) == 0
	      ...
	      NDPI_LOG__DEBUG2(ndpi_struct, "found MSN with pattern text/sml; charset0utf-8\n");
	      NDPI_LOG__TRACE(ndpi_struct, "found MSN xml CVS / VER / ANS found\n");
	      ndpi_int_msn_add_connection(ndpi_struct, flow);
	      return;
	    }
	  }
	...
	if(ndpi_int_find_xmsn(ndpi_struct, flow) == 1) {
	  NDPI_LOG__TRACE(ndpi_struct, "found MSN X-MSN\n");
	  ndpi_int_msn_add_connection(ndpi_struct, flow);
	  return;
	}
      }
	...
	NDPI_LOG__TRACE(ndpi_struct, "found MSN  with pattern CONNECT messenger.hotmail.com:1863 HTTP/1.\n");
	ndpi_int_msn_add_connection(ndpi_struct, flow);
	return;
      }
    }
	...
	while (plen < endlen) {
	  if (ndpi_check_for_email_address(ndpi_struct, flow, plen) != 0) {
	    NDPI_LOG__DEBUG2(ndpi_struct, "found mail address\n");
	    break;
	  }
	...
	NDPI_LOG__TRACE(ndpi_struct, "found MSN  with pattern USR/ANS ...mail_address\n");
	ndpi_int_msn_add_connection(ndpi_struct, flow);
	return;
      }
    }
  ...
  if(flow->packet_counter <= 10 &&
	...
	NDPI_LOG__TRACE(ndpi_struct, "found MSN application/x-msn-messenger\n");
	ndpi_int_msn_add_connection(ndpi_struct, flow);
	return;
      }
      if(ndpi_int_find_xmsn(ndpi_struct, flow) == 1) {
  ...
  if((src != NULL && NDPI_COMPARE_PROTOCOL_TO_BITMASK(src->detected_protocol_bitmask, NDPI_PROTOCOL_MSN) != 0)
	...
	packet->payload_packet_len > 12 && memcmp(packet->payload, "recipientid=", 12) == 0) {
      NDPI_LOG__TRACE(ndpi_struct, "found MSN file transfer\n");
      ndpi_int_msn_add_connection(ndpi_struct, flow);
      return;
    }
  }
  ...
  if(flow->l4.tcp.msn_stage == 0) {
    ...
    if((packet->payload_packet_len == 4 || packet->payload_packet_len == 8)
      ...
      NDPI_LOG__DEBUG2(ndpi_struct, "maybe first TCP MSN detected\n");
    ...
    if (packet->payload_packet_len > 10 && get_u_int32_t(packet->payload, 0) == htonl(0x666f6f00)) {
      ndpi_int_msn_add_connection(ndpi_struct, flow);
      NDPI_LOG__TRACE(ndpi_struct, "found MSN File Transfer 1\n");
      return;
    }
    /* did not see this pattern in any trace */
    if (packet->payload_packet_len == 56 && get_u_int32_t(packet->payload, 16) == 0) {
      NDPI_LOG__DEBUG2(ndpi_struct, "maybe Second TCP MSN detected\n");
    ...
    ndpi_int_msn_add_connection(ndpi_struct, flow);
    NDPI_LOG__TRACE(ndpi_struct, "found MSN File Transfer 2\n");
    return;
	...
	(get_u_int32_t(packet->payload, 0) == htonl(0x10000000) || get_u_int32_t(packet->payload, 0) == htonl(0x30000000))) {
      ndpi_int_msn_add_connection(ndpi_struct, flow);
      NDPI_LOG__TRACE(ndpi_struct, "found MSN File Transfer 3\n");
      return;
    }
  ...
  NDPI_LOG__DEBUG(ndpi_struct, "msn 7\n");
  ...
  if (flow->packet_counter <= MAX_PACKETS_FOR_MSN) {	
	...
	|| memcmp(&packet->payload[0], "XFR ", 4) == 0) {
      ...
      NDPI_LOG__TRACE(ndpi_struct, "found MSN\n");
      return;
    }
  }
 ndpi_msn_exclude:
				     ...
				     *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (packet->payload_packet_len == 20
    ...
    NDPI_LOG__TRACE(ndpi_struct, "found MSN udp misc data connection\n");
    ndpi_int_msn_add_connection(ndpi_struct, flow);
  }
...
void ndpi_search_msn(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG__DEBUG(ndpi_struct, "search msn\n");
