...
void ndpi_search_netbios(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG__DEBUG(ndpi_struct, "search netbios\n");
  if(packet->udp != NULL) {
	...
	h.authority_rrs = ntohs(h.authority_rrs), h.additional_rrs = ntohs(h.additional_rrs);
      ...
      NDPI_LOG__DEBUG(ndpi_struct, "found netbios port 137 and payload_packet_len 50\n");
	...
	NDPI_LOG__TRACE(ndpi_struct, "found netbios with questions = 1 and answers = 0, authority = 0  \n");
	...
	NDPI_LOG__TRACE(ndpi_struct, "found netbios with questions = 1 and answers = 0, authority = 0 and broadcast \n");
	...
	NDPI_LOG__TRACE(ndpi_struct, "found netbios with questions = 1 and answers, authority, additional = 0  \n");
	...
	NDPI_LOG__DEBUG2(ndpi_struct, "possible netbios name query request\n");
	...
	ntohs(get_u_int16_t(packet->payload, 10)) == packet->payload_packet_len - 14) {
      ...
      NDPI_LOG__DEBUG2(ndpi_struct, "found netbios port 138 and payload length >= 112 \n");

      if(packet->payload[0] >= 0x11 && packet->payload[0] <= 0x16) {

	NDPI_LOG__DEBUG2(ndpi_struct, "found netbios with MSG-type 0x11,0x12,0x13,0x14,0x15 or 0x16\n");

	if(ntohl(get_u_int32_t(packet->payload, 4)) == ntohl(packet->iph->saddr)) {
	  NDPI_LOG__TRACE(ndpi_struct, "found netbios with checked ip-address\n");
  ...
  if(packet->tcp != NULL) {
    ...
    if(dport == 139) {
      ...
      NDPI_LOG__DEBUG2(ndpi_struct, "found netbios with destination port 139\n");

      /* payload_packet_len must be 72 */
      if(packet->payload_packet_len == 72) {
	NDPI_LOG__DEBUG2(ndpi_struct, "found netbios with payload_packen_len = 72. \n");
