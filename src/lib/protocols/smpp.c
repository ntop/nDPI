                          ...
                          struct ndpi_flow_struct* flow)
{
  ...
  if (flow->packet.detected_protocol_stack[0] != NDPI_PROTOCOL_SMPP){
    ...
    // get PDU length
    uint32_t pdu_l = ntohl(get_u_int32_t(packet->payload, 0));
    ...
    if(pdu_l != packet->payload_packet_len) {
        ...
        uint32_t total_pdu_l = pdu_l;
        uint32_t tmp_pdu_l = 0;
        uint16_t pdu_c = 1;
    ...
    // *** check PDU type ***
    uint32_t pdu_type = ntohl(get_u_int32_t(packet->payload, 4));
    ...
    uint32_t pdu_req = pdu_type & 0x00FFFFFF;
