...
{
  NDPI_LOG__DEBUG(ndpi_struct, "search Starcraft\n");
  if (flow->packet.detected_protocol_stack[0] != NDPI_PROTOCOL_STARCRAFT) {
    ...
    if (packet->udp != NULL) {
      ...
      if (result == 1) {
	NDPI_LOG__TRACE(ndpi_struct, "Found Starcraft 2 [Game, UDP]\n");
	...
	return;
      ...
      if (result == 1) {
	NDPI_LOG__TRACE(ndpi_struct, "Found Starcraft 2 [Client, TCP]\n");
        ndpi_set_detected_protocol(ndpi_struct, flow, NDPI_PROTOCOL_STARCRAFT, NDPI_PROTOCOL_UNKNOWN);
	return;
      }
    }
