			      ...
			      *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG__DEBUG(ndpi_struct, "search RTSP\n");
  ...
  if (flow->rtsprdt_stage == 0
    ...
    NDPI_LOG__DEBUG2(ndpi_struct, "maybe handshake 1; need next packet, return\n");
    return;
  }

  if (flow->packet_counter < 3 && flow->rtsprdt_stage == 1 + packet->packet_direction) {

    NDPI_LOG__DEBUG2(ndpi_struct, "maybe handshake 2; need next packet\n");
    return;
  }
