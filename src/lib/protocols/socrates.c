			  ...
			  *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG__DEBUG(ndpi_struct, "search socrates\n");

  if (packet->udp != NULL) {
    if (packet->payload_packet_len > 9 && packet->payload[0] == 0xfe
	&& packet->payload[packet->payload_packet_len - 1] == 0x05) {
      NDPI_LOG__DEBUG2(ndpi_struct, "found fe\n");

      NDPI_LOG__DEBUG2(ndpi_struct, "len match\n");
      if (memcmp(&packet->payload[2], "socrates", 8) == 0) {
	NDPI_LOG__TRACE(ndpi_struct, "found socrates udp\n");
	...
	if (memcmp(&packet->payload[6], "socrates", 8) == 0) {
	  NDPI_LOG__TRACE(ndpi_struct, "found socrates tcp\n");
	  ndpi_socrates_add_connection(ndpi_struct, flow);
	}
      }
    }
