...
u_int8_t ndpi_int_zattoo_user_agent_set(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  if(flow->packet.user_agent_line.ptr != NULL && flow->packet.user_agent_line.len == 111) {
    if(memcmp(flow->packet.user_agent_line.ptr + flow->packet.user_agent_line.len - 25, "Zattoo/4", sizeof("Zattoo/4") - 1) == 0) {
      NDPI_LOG__DEBUG(ndpi_struct, "found zattoo useragent\n");
      return 1;
    }
  }
  return 0;
...
void ndpi_search_zattoo(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  NDPI_LOG__DEBUG(ndpi_struct, "search ZATTOO\n");
  ...
  if(packet->tcp != NULL) {
    if(packet->payload_packet_len > 50 && memcmp(packet->payload, "GET /frontdoor/fd?brand=Zattoo&v=", 33) == 0) {
      ...
      NDPI_LOG__TRACE(ndpi_struct, "found zattoo. add connection over tcp with pattern GET /frontdoor/fd?brand=Zattoo&v=\n");
      ZATTOO_DETECTED;
      return;
    }
    if(packet->payload_packet_len > 50	&& memcmp(packet->payload, "GET /ZattooAdRedirect/redirect.jsp?user=", 40) == 0) {
      
      NDPI_LOG__TRACE(ndpi_struct, "found zattoo. add connection over tcp with pattern GET /ZattooAdRedirect/redirect.jsp?user=\n");
      ZATTOO_DETECTED;
      return;
    }
	...
	if(packet->line[i].len >= 18 && (memcmp(packet->line[i].ptr, "User-Agent: Zattoo", 18) == 0)) {
	  ...
	  NDPI_LOG__TRACE(ndpi_struct, "found zattoo. add connection over tcp with pattern POST /channelserver/player/channel/update HTTP/1.1\n");
	  ZATTOO_DETECTED;
	  return;
	}
      }
	...
	NDPI_LOG__TRACE(ndpi_struct, "found zattoo. add connection over tcp with pattern GET / or POST /\n");
	ZATTOO_DETECTED;
	return;
      }
	...
	if(ip == packet->iph->daddr
	  ...
	  NDPI_LOG__TRACE(ndpi_struct, "found zattoo. add connection over tcp with pattern POST http://\n");
	  ZATTOO_DETECTED;
	  return;
	}
      }
	...
	NDPI_LOG__DEBUG2(ndpi_struct, "need next packet, seen pattern 0x030400040a00\n");
	return;
      }
      ...
      NDPI_LOG__TRACE(ndpi_struct, "found zattoo. add connection over tcp with 0x0304\n");
      ZATTOO_DETECTED;
      return;
	...
	NDPI_LOG__DEBUG2(ndpi_struct, "need next packet, seen pattern 0x0000\n");
	return;
      }
      if(packet->payload_packet_len > 50
	  && packet->payload[0] == 0x03
      ...
      NDPI_LOG__TRACE(ndpi_struct, "found zattoo\n");
      ZATTOO_DETECTED;
      return;
  ...
  /* search over UDP */
	...
	NDPI_LOG__TRACE(ndpi_struct, "found zattoo. add connection over udp\n");
	ZATTOO_DETECTED;
	return;
      }
      NDPI_LOG__DEBUG2(ndpi_struct, "need next packet udp\n");
      return;
    }
