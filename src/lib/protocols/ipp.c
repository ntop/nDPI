...
void ndpi_search_ipp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	NDPI_LOG__DEBUG(ndpi_struct, "search ipp\n");
	...
	if (packet->payload_packet_len > 20) {
		...
		if (packet->payload[i] < '0' || packet->payload[i] > '9') {
			NDPI_LOG__DEBUG2(ndpi_struct, "payload does not begin with a number\n");
		...
		if (packet->payload[i++] != ' ') {
			NDPI_LOG__DEBUG2(ndpi_struct, "there is no blank following the number\n");
			goto search_for_next_pattern;
		}

		if (packet->payload[i] < '0' || packet->payload[i] > '9') {
			NDPI_LOG__DEBUG2(ndpi_struct, "no number following the blank\n");
			goto search_for_next_pattern;
		}
		...
		if (memcmp(&packet->payload[i], " ipp://", 7) != 0) {
			NDPI_LOG__DEBUG2(ndpi_struct, "the string ' ipp://' does not follow\n");
			goto search_for_next_pattern;
		}

		NDPI_LOG__TRACE(ndpi_struct, "found ipp\n");
  ...
  search_for_next_pattern:
	...
	if (packet->payload_packet_len > 3 && memcmp(packet->payload, "POST", 4) == 0) {
		...
		if (packet->content_line.ptr != NULL && packet->content_line.len > 14
			...
			NDPI_LOG__TRACE(ndpi_struct, "found ipp via POST ... application/ipp\n");
			ndpi_int_ipp_add_connection(ndpi_struct, flow);
			return;
		}
	}
