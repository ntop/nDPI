...
{
  ndpi_set_detected_protocol(ndpi_struct, &flow->core, NDPI_PROTOCOL_SOCKS, NDPI_PROTOCOL_UNKNOWN, NDPI_CONFIDENCE_DPI);
}
...
static void ndpi_check_socks4(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if(flow->metadata.l4.tcp.socks4_stage == 0) {
    ...
    if(payload_len >= 9 && packet->payload[0] == 0x04 && 
      ...
      flow->metadata.l4.tcp.socks4_stage = packet->packet_direction + 1;
    }
  } else {
    NDPI_LOG_DBG2(ndpi_struct, "SOCKS4 stage %u: \n", flow->metadata.l4.tcp.socks4_stage);

    /* At first check, if this is for sure a response packet (in another direction. If not, do nothing now and return. */
    if((flow->metadata.l4.tcp.socks4_stage - packet->packet_direction) == 1) {
      return;
    }
    ...
    if(payload_len == 8 && packet->payload[0] == 0x00 && packet->payload[1] >= 0x5a && packet->payload[1] <= 0x5d) {
      ...
      flow->metadata.l4.tcp.socks4_stage = 0;
    }
  }
}
...
static void ndpi_check_socks5(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  /* Check if we so far detected the protocol in the request or not. */
  if(flow->metadata.l4.tcp.socks5_stage == 0) {
    ...
    if(((payload_len == 3) && (packet->payload[0] == 0x05) && (packet->payload[1] == 0x01) && (packet->payload[2] == 0x00)) ||
      ...
      /* Encode the direction of the packet in the stage, so we will know when we need to look for the response packet. */
      flow->metadata.l4.tcp.socks5_stage = packet->packet_direction + 1;
    }

  } else {
    NDPI_LOG_DBG2(ndpi_struct, "SOCKS5 stage %u: \n", flow->metadata.l4.tcp.socks5_stage);

    /* At first check, if this is for sure a response packet (in another direction. If not, do nothing now and return. */
    if((flow->metadata.l4.tcp.socks5_stage - packet->packet_direction) == 1) {
      return;
    }
    ...
    if((payload_len == 0) || ((payload_len == 2) && (packet->payload[0] == 0x05) && (packet->payload[1] == 0x00))) {
      ...
      flow->metadata.l4.tcp.socks5_stage = 0;
    }
