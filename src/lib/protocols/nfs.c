...
void ndpi_search_nfs(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	NDPI_LOG__DEBUG(ndpi_struct, "search NFS\n");
	...
	NDPI_LOG__DEBUG2(ndpi_struct, "NFS user match stage 1\n");


	if (offset != 0 && get_u_int32_t(packet->payload, 0) != htonl(0x80000000 + packet->payload_packet_len - 4))
		goto exclude_nfs;

	NDPI_LOG__DEBUG2(ndpi_struct, "NFS user match stage 2\n");

	if (get_u_int32_t(packet->payload, 4 + offset) != 0)
		goto exclude_nfs;

	NDPI_LOG__DEBUG2(ndpi_struct, "NFS user match stage 3\n");

	if (get_u_int32_t(packet->payload, 8 + offset) != htonl(0x02))
		goto exclude_nfs;

	NDPI_LOG__DEBUG2(ndpi_struct, "NFS match stage 3\n");
	...
	NDPI_LOG__DEBUG2(ndpi_struct, "NFS match stage 4\n");

	if (ntohl(get_u_int32_t(packet->payload, 16 + offset)) > 4)
		goto exclude_nfs;

	NDPI_LOG__TRACE(ndpi_struct, "found NFS\n");
