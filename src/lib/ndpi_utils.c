...
int ndpi_dpi2json(struct ndpi_detection_module_struct *ndpi_struct,
  ...
  case NDPI_PROTOCOL_DNS:
    ...
    if(flow->host_server_name[0] != '\0' && flow->http.ishost )
  ...
  case NDPI_PROTOCOL_HTTP:
    ...
    if(flow->host_name[0] != '\0')
      ndpi_serialize_string_string(serializer, "host", (const char*)flow->host_name);
    if(flow->server_name[0] != '\0')
      ndpi_serialize_string_string(serializer, "server", (const char*)flow->server_name);
...
void ins_sort_array(sorter_index_item_t arr[], int len) {
  ...
  for (int i=1; i<len; i++) {
    for (int j=i; j>0 && arr[j].sort_value<arr[j-1].sort_value; j--) {
      if(arr[j].sort_value < arr[j-1].sort_value){
...
uint32_t reassembly_fragment(struct ndpi_flow_struct *const flow, struct ndpi_tcphdr const * tcph, uint8_t **ret_buffer, size_t *len_buffer)
{
  ...
  if ( last_item==fragW->ct_frag ) {
    ...
    for (int i=last_item; i<fragW->ct_frag; i++) {
