...
void ndpi_set_proto_defaults(struct ndpi_detection_module_struct *ndpi_mod,
  ...
  if(protoId >= NDPI_MAX_SUPPORTED_PROTOCOLS+NDPI_MAX_NUM_CUSTOM_PROTOCOLS) {
#ifdef DEBUG
    NDPI_LOG__ERROR(ndpi_mod, "[NDPI] %s/protoId=%d: INTERNAL ERROR\n", protoName, protoId);
#endif
    return;
  }
...
#ifdef DEBUG
    NDPI_LOG__ERROR(ndpi_mod, "[NDPI] %s/protoId=%d: already initialized. Ignoring it\n", protoName, protoId);
#endif
    return;
  }
			   ...
			   const char *_func, int _line)
{
  ...
  for(port=range->port_low; port<=range->port_high; port++) {
    ...
    if(!node) {
      NDPI_LOG__ERROR(ndpi_mod, "%s:%d not enough memory\n", _func, _line);
      break;
    }
    ...
    if(ret != node) {
      NDPI_LOG__DEBUG(ndpi_mod, "[NDPI] %s:%d found duplicate for port %u: overwriting it with new value\n",
		      _func, _line, port);
				 ...
				 ndpi_protocol_breed_t breed)
{
  ...
  if(protocol_id >= (NDPI_MAX_SUPPORTED_PROTOCOLS+NDPI_MAX_NUM_CUSTOM_PROTOCOLS)) {
    NDPI_LOG__ERROR(ndpi_struct, "[NDPI] protoId=%d: INTERNAL ERROR\n", protocol_id);
    return(-1);
  }
...
static int ndpi_remove_host_url_subprotocol(struct ndpi_detection_module_struct *ndpi_struct,
					    char *value, int protocol_id)
{
  NDPI_LOG__ERROR(ndpi_struct, "[NDPI] Missing implementation for proto %s/%d\n",value,protocol_id);
  return(-1);
}
...
static void ndpi_init_protocol_defaults(struct ndpi_detection_module_struct *ndpi_mod) {
  ...
  u_int16_t no_master[2] = { NDPI_PROTOCOL_NO_MASTER_PROTO, NDPI_PROTOCOL_NO_MASTER_PROTO },
	...
	NDPI_LOG__ERROR(ndpi_mod, "[NDPI] missing protoId=%d/%s: INTERNAL ERROR: not all protocols have been initialized\n",
...
struct ndpi_detection_module_struct *ndpi_init_detection_module(void) {
  ...
  if(ndpi_str == NULL) {
#ifdef NDPI_ENABLE_DEBUG_MESSAGES
    NDPI_LOG__ERROR(ndpi_str, "ndpi_init_detection_module initial malloc failed for ndpi_str\n");
...
int ndpi_handle_rule(struct ndpi_detection_module_struct *ndpi_mod, char* rule, u_int8_t do_add) {
  ...
  at = strrchr(rule, '@');
  if(at == NULL) {
    NDPI_LOG__ERROR(ndpi_mod, "Invalid rule '%s'\n", rule);
    return(-1);
  } else
    at[0] = 0, proto = &at[1];
  ...
  if(def == NULL) {
    if(!do_add) {
      ...
      NDPI_LOG__ERROR(ndpi_mod, "Unable to find protocol '%s': skipping rule '%s'\n", proto, rule);
      return(-3);
    } else {
      ndpi_port_range ports_a[MAX_DEFAULT_PORTS], ports_b[MAX_DEFAULT_PORTS];
      u_int16_t no_master[2] = { NDPI_PROTOCOL_NO_MASTER_PROTO, NDPI_PROTOCOL_NO_MASTER_PROTO };
	...
	NDPI_LOG__ERROR(ndpi_mod, "Too many protocols defined (%u): skipping protocol %s\n",
...
int ndpi_load_protocols_file(struct ndpi_detection_module_struct *ndpi_mod, char* path) {
  ...
  if(fd == NULL) {
    NDPI_LOG__ERROR(ndpi_mod, "Unable to open file %s [%s]", path, strerror(errno));
    return(-1);
  }
					  ...
					  const NDPI_PROTOCOL_BITMASK * dbm)
{
  ...
  for(a = 0; a < ndpi_struct->callback_buffer_size; a++) {
	...
	& (NDPI_SELECTION_BITMASK_PROTOCOL_INT_TCP |
      ...
      if(_ndpi_debug_callbacks) NDPI_LOG__DEBUG2(ndpi_struct,
					       ...
					       u_int8_t * l4_protocol_return, u_int32_t flags)
{
  ...
  if(iph->version == IPVERSION && iph->ihl >= 5) {
    NDPI_LOG__DEBUG2(ndpi_struct, "ipv4 header\n");
  }
#ifdef NDPI_DETECTION_SUPPORT_IPV6
  else if(iph->version == 6 && l3_len >= sizeof(struct ndpi_ipv6hdr)) {
    NDPI_LOG__DEBUG2(ndpi_struct, "ipv6 header\n");
  ...
  if((flags & NDPI_DETECTION_ONLY_IPV6) && iph != NULL) {
    NDPI_LOG__DEBUG2(ndpi_struct, "ipv4 header found but excluded by flag\n");
    return 1;
  }
#ifdef NDPI_DETECTION_SUPPORT_IPV6
  else if((flags & NDPI_DETECTION_ONLY_IPV4) && iph_v6 != NULL) {
    NDPI_LOG__DEBUG2(ndpi_struct, "ipv6 header found but excluded by flag\n");
    return 1;
  }
#endif
...
void check_ndpi_udp_flow_func(struct ndpi_detection_module_struct *ndpi_struct,
  ...
  for(a = 0; a < ndpi_struct->callback_buffer_size_udp; a++) {
    if((func != ndpi_struct->callback_buffer_udp[a].func)
      ...
      // NDPI_LOG__DEBUG(ndpi_struct, "[UDP,CALL] dissector of protocol as callback_buffer idx =  %d\n",a);
      if(flow->detected_protocol_stack[0] != NDPI_PROTOCOL_UNKNOWN)
	break; /* Stop after detecting the first protocol */
    } else
      if(_ndpi_debug_callbacks) NDPI_LOG__DEBUG2(ndpi_struct,
				       ...
				       struct ndpi_flow_struct *flow, u_int16_t counter)
{
  ...
  NDPI_LOG__DEBUG2(ndpi_struct, "called ndpi_check_for_email_address\n");
  ...
  if(packet->payload_packet_len > counter && ((packet->payload[counter] >= 'a' && packet->payload[counter] <= 'z')
    ...
    NDPI_LOG__DEBUG2(ndpi_struct, "first letter\n");
    counter++;
    while (packet->payload_packet_len > counter
      ...
      NDPI_LOG__DEBUG2(ndpi_struct, "further letter\n");
      counter++;
	...
	while (packet->payload_packet_len > counter
		   ...
		   || (packet->payload[counter] >= '0' && packet->payload[counter] <= '9')
		   || packet->payload[counter] == '-' || packet->payload[counter] == '_')) {
	  NDPI_LOG__DEBUG2(ndpi_struct, "letter\n");
	  counter++;
	  if(packet->payload_packet_len > counter && packet->payload[counter] == '.') {
	    NDPI_LOG__DEBUG2(ndpi_struct, ".\n");
	    counter++;
		...
		if(packet->payload_packet_len > counter
		  ...
		  if(packet->payload_packet_len > counter
		     && (packet->payload[counter] == ' ' || packet->payload[counter] == ';')) {
		    NDPI_LOG__DEBUG2(ndpi_struct, "whitespace3\n");
		    return counter;
		  } else {
		    return 0;
		  }
...
static int ndpi_automa_match_string_subprotocol(struct ndpi_detection_module_struct *ndpi_struct,
...
#ifdef DEBUG
  {
    ...
    NDPI_LOG__DEBUG2(ndpi_struct, "[NDPI] ndpi_match_host_subprotocol(%s): %s\n",
  ...
  NDPI_LOG__DEBUG2(ndpi_struct, "[NTOP] Unable to find a match for '%s'\n", string_to_match);
#endif
