...
def process_file():
    ...
    with open(TEMP_FILE, "r") as f:
        for line in f:
            ...
            if len(parts) == 1:
                # BinaryDefense publishes one mining-pool domain per line.
                domain = parts[0]
            else:
...
def save_lists(domains, ips):
    ...
    # The replacement source is domain-only. Preserve a previously generated
    # IP list instead of silently deleting IP coverage when no IPs are supplied.
    ip_time = 0
    if ips:
        start_time = time.time()
