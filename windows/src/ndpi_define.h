...
#define MAX_DEFAULT_PORTS                                        5
 ...
 /* Global log, expected to be called only from global context code */
 #define NDPI_GLOG(g_ctx, log_level, args...)		                                 \
  {								                         \
    if(g_ctx && g_ctx->ndpi_log_level >= log_level) {	                                 \
      if(g_ctx != NULL && g_ctx->ndpi_debug_printf != NULL)		                         \
        (*(g_ctx->ndpi_debug_printf))(NDPI_PROTOCOL_UNKNOWN, g_ctx, log_level, __FILE__, __FUNCTION__, __LINE__, args); \
    } \
  }
 ...
 #define NDPI_LOG_ERR(mod, args...)		                                 \
  if(mod && mod->g_ctx && mod->g_ctx->ndpi_log_level >= NDPI_LOG_ERROR) {	                         \
    if(mod->g_ctx->ndpi_debug_printf != NULL)		                         \
      (*(mod->g_ctx->ndpi_debug_printf))(NDPI_CURRENT_PROTO, mod, NDPI_LOG_ERROR , __FILE__, __FUNCTION__, __LINE__, args); \
  }

 #define NDPI_LOG_INFO(mod, args...)		                                 \
  if(mod && mod->g_ctx && mod->g_ctx->ndpi_log_level >= NDPI_LOG_TRACE) {	                         \
    if(mod->g_ctx->ndpi_debug_printf != NULL)		                         \
      (*(mod->g_ctx->ndpi_debug_printf))(NDPI_CURRENT_PROTO, mod, NDPI_LOG_TRACE , __FILE__, __FUNCTION__, __LINE__, args); \
  }

 #define NDPI_LOG_DBG(mod, args...)		                                 \
  if(mod && mod->g_ctx && mod->g_ctx->ndpi_log_level >= NDPI_LOG_DEBUG) {	                         \
    if(mod->g_ctx->ndpi_debug_printf != NULL)		                         \
      (*(mod->g_ctx->ndpi_debug_printf))(NDPI_CURRENT_PROTO, mod, NDPI_LOG_DEBUG , __FILE__, __FUNCTION__, __LINE__, args); \
  }

 #define NDPI_LOG_DBG2(mod, args...)		                                 \
  if(mod && mod->g_ctx && mod->g_ctx->ndpi_log_level >= NDPI_LOG_DEBUG_EXTRA) {	                         \
    if(mod->g_ctx->ndpi_debug_printf != NULL)		                         \
      (*(mod->g_ctx->ndpi_debug_printf))(NDPI_CURRENT_PROTO, mod, NDPI_LOG_DEBUG_EXTRA , __FILE__, __FUNCTION__, __LINE__, args); \
  }
...
# else
# define NDPI_GLOG(mod, log_level, args...) { /* printf(args); */ }
